...
def convert(decimal_num):
    ...
    for i in roman.keys():
        num_to_roman += roman[i] * (
            decimal_num // i)  # (decimal_num//i) gives us quotient of division
...
def main_post():
    if request.method == 'POST':
        ...
        if not alpha.isdecimal():
            return render_template('index.html', developer_name='Jason', not_valid=True)
        number=int(alpha)
        if not 0 < number < 4000:
            return render_template('index.html', developer_name='Jason', not_valid=True)
        return render_template('result.html', number_decimal=number, number_roman=convert(number), developer_name='Jason'
    
    else:
        return render_template('index.html',
                               developer_name='Jason',
                               not_valid=False)
